...
# Manually defining Signal to avoid numpy dependency in test
class Signal:
    def __init__(self, type: str, price: float, timestamp: float, confidence: float, metadata: Dict[str, Any] = None):
        self.type = type
        self.price = price
...
class MockStrategy(BaseStrategy):
    def __init__(self, signal_type='HOLD', confidence=0.0):
        super().__init__()
...
class TestStrategies(unittest.TestCase):
    def setUp(self):
        self.data = {
            'close': [100.0, 101.0, 102.0, 99.0, 98.0, 103.0],
            'high': [105.0, 106.0, 107.0, 101.0, 100.0, 108.0],
            'low': [95.0, 96.0, 97.0, 97.0, 96.0, 100.0],
    ...
    def test_risk_managed_stop_loss(self):
        ...
        self.assertIsNotNone(risk_manager.active_position)

        sig1 = risk_manager.next(1)
        self.assertEqual(sig1.type, 'BUY')

        # Force a crash
        self.data['close'] = [100.0, 101.0, 102.0, 80.0, 79.0, 103.0]
        risk_manager.init(self.data)
        risk_manager.next(0)
    ...
    def test_meta_ensemble_weighting(self):
        s1 = MockStrategy(signal_type='BUY', confidence=1.0)
